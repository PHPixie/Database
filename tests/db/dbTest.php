...
class DBTest extends PHPUnit_Framework_TestCase {
	...
	public function testBuildDriver() {
		foreach(array('PDO', 'Mongo') as $driver_name){
			$driver = $this->db->build_driver($driver_name);
			$this->assertEquals('PHPixie\DB\Driver\\'.$driver_name, get_class($driver));
	...
	public function testDriver() {
		foreach(array('PDO', 'Mongo') as $driver_name){
			$driver = $this->db->driver($driver_name);
			$this->assertEquals('PHPixie\DB\Driver\\'.$driver_name, get_class($driver));
			$this->assertAttributeEquals($this->db,'db',$driver);
			$driver2 = $this->db->driver($driver_name);
